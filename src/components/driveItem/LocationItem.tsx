...
interface LocationItemProps {
    ...
    onDoubleClik?(locationItem: DriveItemModel): void
};

const LocationItem = ({ items, onDoubleClik }: LocationItemProps) => {
    ...
    const onDoubleClikHandle = (item: DriveItemModel) => {
        if (onDoubleClik) {
            onDoubleClik(item);
            return
        }

        openFolder(item.item_Rid);
    }
    ...
    return (
        <>
            ...
            <div style={{ display: "grid", gridGap: 0, gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))" }}>
                {
                    items.map(item => (
                        <div key={item.item_Rid} style={{ display: "block" }}>
                            <div style={{ display: "flex", padding: "7px", position: "relative", maxWidth: "100%" }}>
                                <div style={{ display: "flex", flex: "1 1 auto", outline: "none", overflow: "hidden" }}>
                                    <div style={{ border: "1px solid #dadce0", borderRadius: "6px", display: "flex", flex: "1 1 auto", flexDirection: "column", overflow: "hidden", position: "relative" }} onDoubleClick={() => { onDoubleClikHandle(item) }}>
